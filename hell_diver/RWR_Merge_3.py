...
problem_path2 = ["./weapons\hd_side_p2_peacemaker_version.weapon"]
# base文件誊写
...
problem_of_single_merge = ["./weapons\_acg_all_throwables.xml"]
        ...
        if(root.attrib):
            if("file" in root.attrib):
                if(root.attrib["file"] not in not_base_file and root.attrib["file"]!=""):
                    root = base_file_change(root,possible_files[root.attrib["file"]])
...
problem_path = ["./weapons\acg_incomparable_damage.projectile"]
#处理单个文件
    ...
    for root, dirs, files in os.walk("./"):
        for file in files:
            ...
            if "模板" in file_path or \
                file.startswith("merged_"): continue
