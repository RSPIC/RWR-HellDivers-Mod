...
abstract class Tracker {
	...
	void handleEvent(const XmlElement@ event) {
		...
		string senderId = event.getStringAttribute("player_id");
		string cid = event.getStringAttribute("character_id");
		string message = event.getStringAttribute("message");
		string position = event.getStringAttribute("position");
		string key = event.getStringAttribute("key");
		string aim_target = event.getStringAttribute("aim_target");
		string query_id = event.getStringAttribute("query_id");
		string TagName = event.getStringAttribute("TagName");
		string faction_id = event.getStringAttribute("faction_id");
		string profile_hash = event.getStringAttribute("profile_hash");
		
		//输出玩家信息
		_log("tracker_player rootName: " + rootName, 1);
		_log("tracker_player name: " + sender, 1);
		_log("tracker_player senderid " + senderId, 1);
		_log("tracker_player cid: " + cid, 1);
		_log("tracker_player message: " + message, 1);
		_log("tracker_player position: " + position, 1);
		_log("tracker_player key: " + key, 1);
		_log("tracker_player aim_target: " + aim_target, 1);
		_log("tracker_player query_id: " + query_id, 1);
		_log("tracker_player TagName: " + TagName, 1);
		_log("tracker_player faction_id: " + faction_id, 1);
		_log("tracker_player profile_hash: " + profile_hash, 1);
		*/
